...
export default function Gallery({ images }: { images?: { imageUrl: string; caption: string; category: string; size: string | null }[] }) {
  const galleryImages = (images && images.length > 0)
    ? images.map(img => ({ src: img.imageUrl, caption: img.caption, category: img.category, size: img.size || 'medium' }))
    : fallbackGallery;
          ...
          <div className="gallery-lightbox__stage" onClick={(e) => e.stopPropagation()}>
            <div className={`gallery-lightbox__image-wrap ${lightboxLoaded ? 'loaded' : ''}`}>
              ...
              <img
                ...
                src={`${galleryImages[activeImage].src.replace(/^\//, '')}`}
