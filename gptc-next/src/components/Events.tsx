...
export default function Events({ events: eventsProp }: { events?: { title: string; description: string; excerpt: string; featureImage: string | null; eventDate: string | null }[] }) {
  const events = (eventsProp && eventsProp.length > 0)
    ? eventsProp.map((ev, i) => {
        const date = ev.eventDate ? new Date(ev.eventDate) : null;
        const months = ['JAN','FEB','MAR','APR','MAY','JUN','JUL','AUG','SEP','OCT','NOV','DEC'];
        const hues = [200, 120, 40, 280, 10, 100];
        return {
          icon: Trophy,
          title: ev.title,
          category: 'Event',
          desc: ev.excerpt || ev.description,
          day: date ? String(date.getDate()) : '—',
          month: date ? months[date.getMonth()] : '—',
          hue: hues[i % hues.length],
          featured: i === 0,
          delay: (i + 1) * 100,
          image: ev.featureImage || '',
        };
      })
    : fallbackEvents;
  return (
