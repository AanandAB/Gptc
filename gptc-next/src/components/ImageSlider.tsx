...
export default function ImageSlider({ slides: slidesProp }: { slides?: { imageUrl: string; title: string; subtitle: string }[] }) {
  const slides = (slidesProp && slidesProp.length > 0)
    ? slidesProp.map(s => ({ src: s.imageUrl, title: s.title, subtitle: s.subtitle }))
    : fallbackSlides;
