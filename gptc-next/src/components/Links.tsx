...
export default function Links({ links }: { links?: { title: string; url: string; category: string }[] }) {
  const importantLinks = links
    ? links.filter(l => l.category === 'important')
    : null;
  const quickLinksData = links
    ? links.filter(l => l.category === 'quick')
    : null;
  return (
            ...
            <ul className="links__list">
              {(importantLinks && importantLinks.length > 0)
                ? importantLinks.map((link, i) => (
                    <li key={i}><a href={link.url} target="_blank" rel="noopener noreferrer"><ChevronRight size={14} /> {link.title}</a></li>
                  ))
                : <>
